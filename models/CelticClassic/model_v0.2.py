...
class SIR(object):
    ...
    def S(self,Stm1,Itm1): #function for susceptible. last week minus random sample of this week
        return Stm1-bin(Stm1, self.beta*Itm1/self.N) #binomial distribution 

    def I(self,Stm1,Itm1): #function for infected. last week + this week - recovered
        return Itm1 + bin( Stm1, Itm1*self.beta/self.N ) - bin(Itm1,self.gamma)

    def R(self,Rtm1,Itm1): #function for recovered, includes death 
        return Rtm1 + bin(Itm1,self.gamma)

    def Smean(self,Stm1,Itm1): #average
    ...
    def generateEpidemic(self,timesteps=10):
        pop = { "S":[self.S0], "I":[self.I0],"R":[self.R0]} #population dictionary with initial values

        for t in range(timesteps):
            Stm1,Itm1,Rtm1 = pop["S"][-1], pop["I"][-1], pop["R"][-1] #give me the last value in the list
        
            St = self.S(Stm1,Itm1) #uses function above to calculate this week's value

            It = self.I(Stm1,Itm1)
            Rt = self.R(Rtm1,Itm1)

            pop['S'].append( St ) #appends the last values just calculated
            pop['I'].append( It )
            pop['R'].append( Rt )
        self.epidemicData = pd.DataFrame(pop) #makes it into a pandas dataframe. Similar to an array
